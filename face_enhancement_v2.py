...
if __name__=='__main__':
    ...
    parser.add_argument('--data_dir_type', type=int, default='1', help='data folder 2 or 3 layers')
    parser.add_argument('--base_dir', type=str, default='./', help='dir of code root')
    ...
    faceenhancer = FaceEnhancement(base_dir=args.base_dir, in_size=args.in_size, out_size=args.out_size, model=args.model, use_sr=args.use_sr, sr_model=args.sr_model, channel_multiplier=args.channel_multiplier, narrow=args.narrow, key=args.key, device='cuda' if args.use_cuda else 'cpu')

    if args.data_dir_type == 1:
        files = sorted(glob.glob(os.path.join(args.indir, '*/*.*g')))
    else:
        files = sorted(glob.glob(os.path.join(args.indir, '*/*/*.*g')))
    # print(os.path.join(args.indir, '*/*.*g'))
    for file in tqdm(files[:]):
        ...
        if args.data_dir_type == 1:
            if not os.path.exists(os.path.join(args.outdir, file.split('/')[-2])):
                os.mkdir(os.path.join(args.outdir, file.split('/')[-2]))
            cv2.imwrite(os.path.join(args.outdir, file.split('/')[-2], filename), img)
        else:
            if not os.path.exists(os.path.join(args.outdir, file.split('/')[-3], file.split('/')[-2])):
                os.makedirs(os.path.join(args.outdir, file.split('/')[-3], file.split('/')[-2]))
            cv2.imwrite(os.path.join(args.outdir, file.split('/')[-3], file.split('/')[-2], filename), img)
